...
const LAYER_CONFIG: Record<string, { layer: string; desc: string }> = {
  ...
  'shared/tool-definitions.ts': { layer: 'shared',  desc: 'UNIVERSAL_TOOLS — frozen tool objects (name, description, inputSchema)' },
  ...
  'daemon/engines/instances.ts': { layer: 'lifecycle', desc: 'Constructs provider adapters and the durable Codex engine' },
  'daemon/engines/codex-surface.ts': { layer: 'lifecycle', desc: 'Codex disposable TUI repair' },
  ...
  'daemon/templates.ts':      { layer: 'domain',    desc: 'Spawn templates (review:, fix:, design:)' },
  ...
  'daemon/commands/review.ts':  { layer: 'commands', desc: 'review command handler' },
  'daemon/commands/build.ts':   { layer: 'commands', desc: 'build command handler' },
  'daemon/commands/design.ts':  { layer: 'commands', desc: 'design command handler' },
