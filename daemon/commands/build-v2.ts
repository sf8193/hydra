...
export async function handleBuildV2Intercept(msg: InboundMessage, rounds: number, task?: string, model?: string): Promise<void> {
  ...
  if (!sessionId) {
    await gateway.send(msg.channelId, `No session owns this thread. Use \`build_v2\` in a session thread.`, { replyTo: msg.id })
    return
  }
  ...
  try {
    ...
    await startProtocolRun(proto, threadId, sessionId, { rounds: clampedRounds, task, model })
  } catch (err) {
