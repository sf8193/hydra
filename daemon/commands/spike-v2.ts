...
export async function handleSpikeV2Intercept(msg: InboundMessage, topic?: string, model?: string, engine?: 'claude' | 'codex'): Promise<void> {
  ...
  if (!sessionId) {
    await gateway.send(msg.channelId, `No session owns this thread. Use \`spike\` in a session thread.`, { replyTo: msg.id })
    return
  }
  ...
  try {
    ...
    await startProtocolRun(proto, threadId, sessionId, { rounds: 1, topic, model, engine })
  } catch (err) {
