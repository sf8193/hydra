...
// Listeners may be sync or async. Async rejections are caught and logged with the listener label.
//
...
export function emit<K extends keyof EventMap>(event: K, payload: EventMap[K]): void {
  ...
  for (const { listener, label } of [...set]) {
    try {
      ...
      if (result !== null && result !== undefined && typeof (result as Promise<void>).catch === 'function') {
