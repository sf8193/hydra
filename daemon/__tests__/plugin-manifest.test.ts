...
describe('bridge declaration', () => {
  ...
  /**
   * A fresh install of the published plugin never runs the daemon's sync, so
   * the repo's own manifest has to carry the declaration to arrive with a bridge.
   */
  test("the repo's own manifest declares the same server", () => {
