...
gateway.onMessage(async (msg: InboundMessage) => {
  ...
  if (isAllowed) {
    ...
    if (threadKillMatch && msg.isThread) {
      void handleThreadKillIntercept, handleDestroyIntercept(msg)
      return
    }
    ...
    // Template as first-class command: "review: topic", "fix: topic", etc.
    // Placed AFTER all hardcoded commands so new commands naturally take priority.
    // Skip in active session threads — let thread-scoped commands (review, build) handle it.
    {
    ...
    if (msg.isThread) {
      ...
      // Transparently strip deprecated _v2 suffix and dispatch the cleaned command
      let cmdContent = msg.content
      if (/^(?:\/?)(?:review_v2|build_v2|spike_v2|kill\s+(?:review_v2|build_v2|spike_v2))\b/i.test(cmdContent)) {
        cmdContent = cmdContent.replace(/^(\/?(?:kill\s+)?(?:review|build|spike))_v2/i, '$1').trim()
      }

      // Reject removed commands explicitly
      if (/^(?:\/?)build-wt(?:[:\s]|$)/i.test(cmdContent)) {
        void gateway.send(msg.channelId, `_\`build-wt\` has been removed. Use \`spawn-wt: repo topic\` then \`build\` in the spawned thread instead._`, { replyTo: msg.id }).catch(() => {})
        return
      }

      const reviewMatch = cmdContent.match(/^(?:\/review|review)(?=[\s:]|$)\s*(?:(\S+?):\s+)?(\d+)?\s*(?:(\S+?):\s+)?([\s\S]+)?$/i)
      if (reviewMatch) {
        const preAlias = reviewMatch[1]?.toLowerCase()
        const postAlias = reviewMatch[3]?.toLowerCase()
        const reviewEngine = (preAlias === 'codex' || postAlias === 'codex') ? 'codex' as const : undefined
        const reviewPreAlias = preAlias === 'codex' ? undefined : preAlias
        const reviewPostAlias = postAlias === 'codex' ? undefined : postAlias
        const preModel = resolveProtocolModel(reviewPreAlias, msg.channelId, msg.id)
        if (preModel === false) return
        const postModel = resolveProtocolModel(reviewPostAlias, msg.channelId, msg.id)
        ...
        if (topic && !reviewPreAlias && !reviewPostAlias) {
        ...
        void handleProtocolIntercept('review', msg, { rounds, topic, model: preModel ?? postModel, modifierNames: mods.length > 0 ? mods : undefined, strike: true, engine: reviewEngine })
        return
      }
      ...
      const buildMatch = cmdContent.match(/^(?:\/build|build)(?=[\s:]|$)\s*(?:(\S+?):\s+)?(\d+)?\s*(?:(\S+?):\s+)?([\s\S]+)?$/i)
      if (buildMatch) {
        const preAlias = buildMatch[1]?.toLowerCase()
        const postAlias = buildMatch[3]?.toLowerCase()
        const buildEngine = (preAlias === 'codex' || postAlias === 'codex') ? 'codex' as const : undefined
        const buildPreAlias = preAlias === 'codex' ? undefined : preAlias
        const buildPostAlias = postAlias === 'codex' ? undefined : postAlias
        ...
        let buildTask = buildMatch[4]?.trim()
        if (buildTask?.startsWith(':')) buildTask = buildTask.slice(1).trim() || undefined
        if (buildTask && !buildPreAlias && !buildPostAlias) {
        ...
        void handleProtocolIntercept('build', msg, { rounds: buildRounds, topic: buildTask, model: preModel ?? postModel, strike: true, engine: buildEngine })
        return
      }
      ...
      const spikeMatch = cmdContent.match(/^(?:\/spike|spike)(?=[\s:]|$)\s*(?:(\S+?):\s+)?([\s\S]+)?$/i)
      if (spikeMatch) {
        const spikeAlias = spikeMatch[1]?.toLowerCase()
        const spikeEngine = spikeAlias === 'codex' ? 'codex' as const : undefined
        const resolvedSpikeAlias = spikeAlias === 'codex' ? undefined : spikeAlias
        const spikeModel = resolveProtocolModel(resolvedSpikeAlias, msg.channelId, msg.id)
        if (spikeModel === false) return
        let spikeTopic = spikeMatch[2]?.trim()
        if (spikeTopic?.startsWith(':')) spikeTopic = spikeTopic.slice(1).trim() || undefined
        void handleProtocolIntercept('spike', msg, { rounds: 1, topic: spikeTopic, model: spikeModel, engine: spikeEngine })
        return
      }

      const cancelSpikeMatch = cmdContent.match(/^\/?(?:kill spike)\s*$/i)
      if (cancelSpikeMatch) {
        void handleCancelProtocolIntercept(msg, 'spike')
        return
      }

      if (/^(?:\/?design\s*:|\/design(?:\s|$)|kill\s+design(?:\s|$))/i.test(cmdContent)) {
        void gateway.send(msg.channelId, `_The \`design\` protocol has been removed. Use \`spawn:\` to start a session and orchestrate design work directly._`, { replyTo: msg.id }).catch(() => {})
        return
      }
