...
async function spawnBuilder(
  ...
  const builderPrompt = [
    ...
    ``,
    `SCOPE DISCIPLINE:`,
    `- Implement EXACTLY what the spec asks. Do not add features, refactors, or abstractions beyond the spec.`,
    `- If the spec says "change X in file Y", touch file Y. Don't reorganize adjacent code.`,
    `- If you think the spec is wrong or incomplete, ask the PM via send_to_thread — don't silently expand scope.`,
    `- A 50-line spec should produce roughly a 50-line change. If your diff is 10x the spec, you've gone rogue — stop and ask.`,
