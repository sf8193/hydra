...
export type RuntimeAdapters = {
  claude: EngineAdapter<'claude'>
  codex: EngineAdapter<'codex'>
}
...
export class SessionRuntime {
  ...
  async resume(provider: ProviderId, input: ProviderRecoveryInput): Promise<(SpawnResult & { bridgeOrphan?: boolean }) | null> {
    ...
    if (provider === 'claude') {
      return this.resumeClaude({
        topic: input.topic, threadId: input.threadId, threadUrl: input.threadUrl,
        claudeSessionId: input.entry?.claudeSessionId ?? input.live?.claudeSessionId,
        model: input.model, worktree: input.worktree, preserveWorktree: input.preserveWorktree,
      })
    }
    ...
    try {
      return await this.spawn(input.topic, undefined, undefined, {
        existingThreadId: input.threadId, ...input.spawnOptions,
        resumeCodex: { threadId, homeName }, promptPrefix: input.recoveryNotice,
        model: input.model, engine: provider, preserveWorktree: input.preserveWorktree,
        reuseWorktree: input.preserveWorktree ? input.worktree : undefined,
      })
    } catch (err) {
      process.stderr.write(`daemon: ${provider} resume failed for ${input.lastName}: ${err}\n`)
      return null
    }
  }

  async fork(provider: ProviderId, input: ProviderRecoveryInput): Promise<SpawnResult | null> {
    if (!this.adapters[provider].capabilities.nativeFork) return null
    const claudeSessionId = input.entry?.claudeSessionId ?? input.live?.claudeSessionId
    const codexThreadId = input.entry?.codexThreadId ?? input.live?.codexThreadId
    if (provider === 'claude' && !claudeSessionId || provider === 'codex' && !codexThreadId) return null
    const options: SpawnOpts = provider === 'claude'
      ? { engine: provider, forkFrom: { claudeSessionId: claudeSessionId!, parentName: input.lastName } }
      : codexForkSpawnOptions({ threadId: codexThreadId!, homeName: input.entry?.codexHomeName ?? input.live?.codexHomeName ?? input.lastName }, input.lastName)
    try {
      return await this.spawn(input.topic, undefined, undefined, {
        existingThreadId: input.threadId, ...input.spawnOptions, ...options, model: input.model,
      })
    } catch (err) {
      if (provider === 'claude') throw err
      process.stderr.write(`daemon: codex fork failed for ${input.lastName}: ${err}\n`)
      return null
    }
  }
  ...
  async spawn(topic: string, chatId?: string, messageId?: string, opts?: SpawnOpts): Promise<SpawnResult> {
        ...
        if (existingId) {
          ...
          if (existing) {
            ...
            if (!alive) {
              ...
              await this.kill(existing, 'replaced by new spawn')
            }
          }
        }
      }
    ...
    if (threadId && !isJoin) {
      ...
      if (existingId) {
        ...
        if (existing) {
          ...
          await this.kill(existing, 'replaced by new spawn', { skipWorktreeDestroy: !!opts?.preserveWorktree })
        }
      }
      if (!anchorMessageId) {
        const thread = threadRegistry.get(threadId)
    ...
    if (engine === 'codex') {
      ...
      opts?.beforeInitialTurn?.(sessionId)
      this.adapters.codex.ensureInteractiveSurface(info)
    ...
    const launched = await this.adapters.claude.spawn({
  ...
  async resumeClaude(dead: {
    ...
    try {
      const result = await this.spawn(dead.topic, undefined, undefined, {
      ...
      if (!ok) {
        ...
        if (verdict === 'kill') {
          ...
          await this.kill(info, 'resume health check failed', { skipWorktreeDestroy: true }).catch(() => {})
          return null
        }
  ...
  async respawn(
    ...
    try {
      return await this.spawn(topic, undefined, undefined, {
        ...extraOpts,
...
export const doSpawnSession = sessionRuntime.spawn.bind(sessionRuntime)
export const killSession = sessionRuntime.kill.bind(sessionRuntime)
export const tryResume = sessionRuntime.resumeClaude.bind(sessionRuntime)
export const tryRespawn = sessionRuntime.respawn.bind(sessionRuntime)
