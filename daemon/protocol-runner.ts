...
const decidingRuns = new Set<string>()
...
export async function startProtocolRun(
  ...
  const lenses = params.lenses as Array<{ lens: string }> | undefined
  const modifiers = lenses?.length ? ` ${lenses.map(l => `+${l.lens}`).join(' ')}` : ''
  const topicLine = params.topic ? `\nFocus: **${params.topic}**${modifiers}` : modifiers ? `\nFocus:${modifiers}` : ''
...
export async function onRunDecision(sessionId: string, value: string, because: string): Promise<boolean> {
  ...
  // Guard against concurrent decide() calls
  if (decidingRuns.has(run.id)) return false
  decidingRuns.add(run.id)
