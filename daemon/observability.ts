...
// Also stamps lastRssMB, so a later autopsy can report memory before death.
export function sessionVitalsLine(info: SessionInfo, now: number): string {
  const v = paneVitals(info.tmuxName)
  if (v.rssMB != null) { info.lastRssMB = v.rssMB; info.lastVitalsAt = now }
  const conn = transport.has(info.sessionId) ? '' : ' [disconnected]'
  return `${info.tmuxName} pid=${v.pid ?? '?'} rss=${v.rssMB ?? '?'}MB up=${dur(now - info.createdAt)} idle=${dur(now - info.lastActive)}${conn}`
}

export function startVitalsSnapshots(): void {
  setInterval(() => {
    const now = Date.now()
    const live = [...registry.values()].filter(s => !s.deadAt)
    if (live.length === 0) return
    const lines = live.map(s => '  ' + sessionVitalsLine(s, now))
...
export function buildAutopsy(info: SessionInfo, reason: string, blackBoxTail: string[]): string {
  const now = Date.now()
  const transcript = info.claudeSessionId ? transcriptPathFor(info.claudeSessionId) : undefined
  const rss = info.lastRssMB != null
    ? `${info.lastRssMB}MB${info.lastVitalsAt ? ` (${dur(now - info.lastVitalsAt)} before death)` : ''}`
    : 'never sampled'
  const lines = [
